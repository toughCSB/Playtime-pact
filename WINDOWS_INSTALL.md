...
Windows x64 PC에서 `Playtime Pact Setup 1.0.2.exe` 파일 하나를 실행합니다. 게임을 먼저 종료하고, 보호 서비스 설치를 위한 Windows 관리자 승인을 진행하세요. 신규 설치의 부모 PIN은 `0000`이므로 관리자 화면의 `부모 PIN 변경`에서 즉시 변경하세요. 시작 승인 설정과 관계없이 하루 두 번째 이후의 새 게임 회차는 PC에서 부모 PIN으로 승인해야 합니다. 이미 PIN으로 승인한 회차의 남은 시간은 다시 승인하지 않고 이어집니다.
