...
[![Version](https://img.shields.io/badge/version-1.0.2-blue)](RELEASE_NOTES_1.0.2.md)
...
현재 Windows 1.0.2 버전은 **공인 코드 서명(Code Signing) 인증서가 적용되지 않은 unsigned 설치본**입니다. Windows Defender SmartScreen, Smart App Control 또는 조직의 App Control 정책이 실행을 경고하거나 완전히 차단할 수 있습니다.

1. 반드시 신뢰할 수 있는 경로로 받은 설치 파일인지 확인합니다.
2. 게임을 모두 종료한 뒤 `Playtime Pact Setup 1.0.2.exe`를 실행합니다.
...
> 1.0.2는 트레이 클릭과 2회차 부모 PIN 승인 경계를 보완하고, 기존 설치의 보호 파일 권한 복구를 강화한 버전입니다. 설치 인증 로직과 신뢰된 배포자 서명은 서로 다른 문제이며, 공인 코드 서명은 아직 없습니다.
...
5. 관리자 화면을 사용하는 동안에는 인증을 유지합니다. 사용을 마치면 **잠금**을 누르세요. 즉시 인증을 폐기하고 창을 최소화합니다.
...
- 트레이 아이콘은 싱글클릭으로 창을 열지 않고, 더블클릭에서만 PIN 잠금이 적용되는 관리 화면을 엽니다.
- 하루 두 번째 이후 게임 회차를 시작할 때 PC 부모 PIN이 필요합니다. PIN 승인 회차의 남은 시간은 게임 재실행·PC 재부팅 후에도 다시 승인하지 않고 이어집니다.
- 기존 설치의 보호 사용량·부모 PIN 파일 권한과 계정 간 공유 기록 권한을 데이터 삭제 없이 복구합니다.
- 자세한 검증 범위와 미완료 사항은 [1.0.2 릴리스 노트](RELEASE_NOTES_1.0.2.md)에 정리했습니다.

## ✅ v1.0.1 버그 수정 요약

- 게임 중 PC를 종료하거나 재부팅해도 꺼져 있던 시간만큼 게임 시간이 차감되지 않도록 보호 사용량을 저장합니다.
- 24시가 지나면 진행 중인 세션을 마감하고 새 날짜의 기본 시간·횟수를 다시 적용합니다.
- 부모 PIN의 5분 유지 제한을 없애고, 관리자 화면의 **잠금** 버튼으로 즉시 잠금·최소화합니다.
- 자세한 동작·검증 범위·미완료 사항은 [1.0.1 릴리스 노트](RELEASE_NOTES_1.0.1.md)에 정리했습니다.
...
최신 1.0.2의 상세 기능 검증 범위는 [릴리스 노트](RELEASE_NOTES_1.0.2.md)를 확인하세요. 아래 표는 0.60.8 전체 회귀검사 기록이며, 이후 설치 복구, 다중 사용자 감지, Minecraft/Lunar 보강 결과는 위에 정리했습니다.
...
- `joonw` 표준 계정에서 1.0.1 설치본으로 Lunar/Minecraft를 실제 실행한 종단 흐름
- 모든 경고 시점의 실제 게임 화면 표시와 월드 저장 무결성
- 신뢰된 코드 서명과 경고 없는 공개 배포
- Android 실기기·FCM 운영·원격 서버 배포
- macOS 설치 및 동작

자동 테스트 통과는 실기기에서 모든 조합이 정상이라는 뜻이 아닙니다. 1.0.2도 위 실기기 검증과 신뢰된 코드 서명은 별도 완료가 필요합니다.
...
| 버전 | 날짜 | 주요 변경 |
|---|---|---|
| **v1.0.2** | 2026-09-26 | 트레이 싱글클릭 차단, 2회차 부모 PIN 승인 및 회차 내 재승인 방지, 설치 권한 복구 |
| **v1.0.1** | 2026-09-25 | 종료·재부팅 시 잔여 시간 보존, 자정 리셋, 부모 PIN 즉시 잠금·최소화 |
