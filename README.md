...
| **Minecraft: Java Edition** | Minecraft로 식별되는 `java.exe` / `javaw.exe` | 실행 명령·설치 경로·런처 계보 감지와 종료 회귀검사 완료 |
| **Minecraft: Bedrock Edition** | Windows용 Minecraft 게임 프로세스 | 감지 로직 포함. 별도 Bedrock 실게임 전체 흐름은 미검증 |
| **Lunar Client** | Lunar Client에서 실행한 Minecraft Java 세션 | 실제 세션 종료 시험과 표준 계정 런타임 실행 정책 확인. rc.9 실게임 재검증 필요 |
...
- Minecraft Launcher, Lunar, Badlion, Feather, Prism, MultiMC, PolyMC, HMCL, PCL2는 런처 창만으로 게임 시간을 차감하지 않음
- `scripts/allow-existing-game-runtimes.ps1`로 표준 계정의 기존 Lunar/Minecraft Java 런타임만 AppLocker에 병합 허용
- 이 PC의 `joonw` 계정에서 Lunar Java 17·21·25의 `java.exe`/`javaw.exe` 6개가 `Allowed`인지 재검증
- Microsoft Store·App Installer·MSI 설치 제한은 변경하지 않음
