...
public final class ApprovalMessagingService extends FirebaseMessagingService {
  ...
  @Override public void onMessageReceived(RemoteMessage message){if(!BuildConfig.FCM_ENABLED)return;FirebaseBootstrap.ensureInitialized(this);sendBroadcast(opaqueWakeupIntent(this,message));notifyWakeup();}
  @Override public void onNewToken(String token){if(!BuildConfig.FCM_ENABLED)return;FirebaseBootstrap.ensureInitialized(this);storeToken(this,token);sendBroadcast(new Intent(ACTION_TOKEN_REFRESH_REQUIRED).setPackage(getPackageName()));}
